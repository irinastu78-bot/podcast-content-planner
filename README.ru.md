...
- **Форматы выпусков:** интервью с гостем, дискуссия двух ведущих,
  соло-монолог, обзор/разбор, сторителлинг, вопрос-ответ.
- **Тон и стиль:** дружелюбный, экспертный, ироничный, мотивирующий,
  академичный, сказочный (с безопасным детским голосом по умолчанию).
- **Текст для озвучки по ролям** (ВЕДУЩИЙ / ГОСТЬ) с накопительным контекстом
  ...
  представления, приветствия в середине, дословные повторы, унифицирует
  обозначения, например E/Z изомеров).
...
- **Контроль качества текста:** автоматическая чистка слов со смешанным
  алфавитом (кириллица + латиница) и диагностика нераспознанных случаев.
...
## Структура проекта
    ...
    │   ├── db_service.py           # SQLite: пользователи, материалы, проекты
