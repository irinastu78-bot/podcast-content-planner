...
def _build_position_block(params: dict, episode: dict,
    ...
    if dialog and partner and partner.lower() not in ("гость", "не указано"):
        guest_intro_rule = ("- ВЕДУЩИЙ представляет собеседника по имени И "
                            "должности/регалиям, взяв их из имени и из "
                            "дополнительных пожеланий, если они заданы: "
                            f"{partner}. Представляет ИМЕННО ведущий — гость НЕ "
                            "представляет себя сам и не называет свою должность "
                            "первым. Это единственное место, где собеседник "
                            "представляется. Не выдумывай отчество, фамилию или "
                            "должность, которых нет ни в имени, ни в пожеланиях.\n"
                            "- После представления гость КОРОТКО отвечает на "
                            "приветствие одной естественной репликой "
                            "(«Здравствуйте, рада быть здесь», «Спасибо, что "
                            "позвали»), и лишь затем переходит к сути. Гость НЕ "
                            "повторяет приветствие ведущего и НЕ представляет "
                            "себя заново.")
